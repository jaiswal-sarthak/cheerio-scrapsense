...
export const ScrapeHealth = ({ items }: { items: Health[] }) => {
  return (
    <Card>
      ...
      <CardContent>
        <div className="max-h-[400px] overflow-y-auto space-y-3 pr-2 scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-700 scrollbar-track-transparent">
          {items.map((item) => (
            <div key={item.id} className="flex items-center justify-between gap-3 rounded-xl border border-white/30 bg-white/50 p-3 backdrop-blur-sm dark:border-white/10 dark:bg-slate-800/40">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-400">
                  {statusCopy[item.status]} {item.lastRun ? `• Last run ${formatToIST(item.lastRun)}` : ""}
                </p>
              </div>
              <Badge
                variant={
